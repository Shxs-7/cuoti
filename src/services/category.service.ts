...
export const categoryService = {
  ...
  async remove(id: string): Promise<void> {
    await db.transaction('rw', [db.categories, db.folders, db.questions, db.reviews], async () => {
      const folders = await db.folders.where('categoryId').equals(id).toArray();
      for (const f of folders) {
        await db.questions.where('folderId').equals(f.id).delete();
      }
