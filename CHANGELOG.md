...
## 2026-08-05 — 错误原因/解析拆分、固定按钮、列表滚动记忆

- 错题字段拆分：原来的"解析"改为"错误原因"，并新增独立的"解析"字段（添加/编辑/详情均支持；旧数据自动把原解析内容迁移为错误原因）。
- 文件夹页「+ 添加错题 / + 添加知识点」按钮改为固定定位，始终显示在屏幕底部导航上方（任何情况下都可见）。
- 列表返回记忆：从文件夹/分类/知识点列表点进详情再返回，停留在原来的滚动位置，不再跳回顶部。

## 2026-08-04 — 标签提示与添加按钮吸底

- 添加/编辑错题时，标签区新增提示：显示当前分类已有的标签，点击即可直接添加（💡 当前分类的标签）。
- 文件夹页的「+ 添加错题 / + 添加知识点」按钮改为吸底固定，错题再多也不用滑到底部。
...
- 添加错题/知识点/日记，以及错题/知识点编辑页的「保存/取消」按钮改为吸底固定，内容再多也不用滑到底部才能保存。

## 2026-08-03（第七次）— 云同步真实状态提示

- 修复「立即同步」永远提示"同步完成"的假成功问题：现在同步失败会明确提示"云端连接异常（Key 失效或项目暂停）"。
- `fullSync()` 现在返回真实结果，按表收集失败信息并记录日志。

## 2026-08-03（第六次）— AI 云端连接诊断

- AI 设置页新增「🔌 测试连接」按钮：一键测试 API 地址/Key/模型是否可用，并直接显示具体原因（HTTP 状态码、错误内容、CORS/网络问题）。
- 云分析失败时，分析结果里直接显示失败原因（HTTP 状态 + 接口返回内容），不再只是笼统的"检查 Key"。
- 提问失败同样显示具体接口错误。

## 2026-08-03（第五次）— 复习图片、设置入口与整体优化

- 复习点「查看答案」后显示**全部图片**（缩略图，可点开全屏）；未看答案时仍只显示第一张。
- 修复设置页没有任何入口的问题：顶栏新增 ⚙️ 设置按钮（每个页面都能进），之前"强制刷新/备份/同步"都藏在设置页里但根本点不到。
- AI 分析失败时显示具体错误原因，云端请求超时从 25 秒缩短到 15 秒。
- 清理无效代码：删除从未使用的 `src/lib/search.ts`；移除文件夹页里没有入口的删除弹窗。
